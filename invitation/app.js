      ...
      const revealLines = Array.from(document.querySelectorAll(".line"));
      ...
      async function init() {
        try {
          ...
          function createFrontEngravingTexture(THREE, text) {
            ...
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.font = "700 140px 'Pretendard Variable', Pretendard, sans-serif";
            ctx.fillStyle = "white";
...
normal = normalize(normal + vec3(-etchSamplePx, 0.0, -etchSamplePy) * (uEtchNormalStrength * etchReveal));`
          ...
          function buildInnerBowlProfile(root, THREE) {
            ...
            const bowlBottomY = yMin + (yMax - yMin) * 0.18;
          ...
          function placeModel(root) {
            ...
            const liquidSurfaceMaterial = new THREE.MeshPhysicalMaterial({
              ...
              attenuationColor: new THREE.Color(0x240104),
            ...
            if (frontTextTexture) {
              ...
              if (document.fonts && document.fonts.load) {
                document.fonts.load("600 170px Pretendard Variable").then(() => {
          ...
          function getScrollProgress() {
            ...
            const max = Math.max(scrollContainer.scrollHeight - scrollContainer.clientHeight, 1);
          ...
          function animate() {
            ...
            if (wine && intro) {
              ...
              camera.lookAt(0.08, 0.02, 0.06);
              ambient.intensity = THREE.MathUtils.lerp(0, 1.6, moveT);
              hemi.intensity = THREE.MathUtils.lerp(0, 1.3, moveT);
              key.intensity = THREE.MathUtils.lerp(0, 2.0, moveT);
              rim.intensity = THREE.MathUtils.lerp(0, 1.1, moveT);
              fill.intensity = THREE.MathUtils.lerp(0, 1.0, moveT);
              if (veil) {
                veil.style.opacity = String(THREE.MathUtils.lerp(0.75, 0, moveT));
              }
              if (t >= 1) {
                intro = null;
              ...
              // Map progress to 4 sections
              // Section 1: Logo (0.0)
              // Section 2: Hero (0.33)
              // Section 3: Story (0.66)
              // Section 4: QR (1.0)
              
              const logoOpacity = THREE.MathUtils.clamp(1 - tRaw * 4, 0, 1);
              if (logoIntroImg) {
                logoIntroImg.style.opacity = String(logoOpacity);
                logoIntroImg.style.transform = `translate3d(0, ${(-tRaw * 40).toFixed(1)}px, 0) scale(${(1 - tRaw * 0.1).toFixed(4)})`;
              }

              // Phase 2 Copy (Hero)
              const copy2Opacity = THREE.MathUtils.clamp(1 - Math.abs(tRaw - 0.33) * 5, 0, 1);
              if (revealCopy) {
                revealCopy.style.opacity = String(copy2Opacity);
                revealCopy.style.filter = `blur(${((1 - copy2Opacity) * 10).toFixed(2)}px)`;
                revealCopy.style.transform = `translate3d(0, ${( (0.33 - tRaw) * 50).toFixed(1)}px, 0)`;
              }

              // Phase 3 Copy (Story)
              const copy3Opacity = THREE.MathUtils.clamp(1 - Math.abs(tRaw - 0.66) * 5, 0, 1);
              if (storyCopy) {
                storyCopy.style.opacity = String(copy3Opacity);
                storyCopy.style.filter = `blur(${((1 - copy3Opacity) * 10).toFixed(2)}px)`;
                storyCopy.style.transform = `translate3d(0, ${( (0.66 - tRaw) * 50).toFixed(1)}px, 0)`;
              }

              // Phase 4 Assets (QR)
              const copy4Opacity = THREE.MathUtils.clamp((tRaw - 0.75) * 4, 0, 1);
              if (endTextImage) {
                endTextImage.style.opacity = String(copy4Opacity);
                endTextImage.style.filter = `blur(${((1 - copy4Opacity) * 10).toFixed(2)}px)`;
                endTextImage.style.transform = `translate3d(0, ${((1 - tRaw) * 40).toFixed(1)}px, 0) scale(${THREE.MathUtils.lerp(0.97, 1, copy4Opacity).toFixed(4)})`;
              }
              if (endQrImage) {
                endQrImage.style.opacity = String(copy4Opacity);
                endQrImage.style.filter = `blur(${((1 - copy4Opacity) * 10).toFixed(2)}px)`;
                endQrImage.style.transform = `translate3d(0, ${((1 - tRaw) * 40).toFixed(1)}px, 0) scale(${THREE.MathUtils.lerp(0.97, 1, copy4Opacity).toFixed(4)})`;
              }

              // --- Animation Mapping for 3D elements ---
              const phaseInRaw = THREE.MathUtils.clamp(tRaw / 0.66, 0, 1);
              const phaseOutRaw = THREE.MathUtils.clamp((tRaw - 0.66) / 0.34, 0, 1);
              const phaseIn = easeInOutCubic(phaseInRaw);
              const phaseOut = easeInOutCubic(phaseOutRaw);
              
              const clinkRaw = THREE.MathUtils.clamp((tRaw - 0.5) / 0.5, 0, 1);
              const finalTurnRaw = THREE.MathUtils.clamp((tRaw - 0.8) / 0.2, 0, 1);
              ...
              const phase1RotY = THREE.MathUtils.lerp(scrollRig.wineRotYFrom, scrollRig.wineRotYMid, phaseIn);
              ...
              if (companionGlass) {
                if (clinkRaw > 0.001) {
                  ...
                  const enter = easeOutCubic(THREE.MathUtils.clamp(clinkRaw / 0.7, 0, 1));
                  const settle = easeInOutCubic(THREE.MathUtils.clamp((clinkRaw - 0.7) / 0.3, 0, 1));
                  ...
                  const companionOpacity = THREE.MathUtils.clamp((clinkRaw - 0.1) * 2, 0, 1);
                  companionGlass.traverse((obj) => {
              ...
              if (clinkTick) {
                const tickRaw = THREE.MathUtils.clamp((clinkRaw - 0.6) / 0.2, 0, 1);
              ...
              if (revealLines.length) {
                for (let i = 0; i < revealLines.length; i += 1) {
                  ...
                  // Check which copy this line belongs to
                  const isPhase2 = revealCopy && revealCopy.contains(line);
                  const isPhase3 = storyCopy && storyCopy.contains(line);
                  const baseOpacity = isPhase2 ? copy2Opacity : (isPhase3 ? copy3Opacity : 0);
                  
                  const local = THREE.MathUtils.clamp(baseOpacity * 1.5 - i * 0.1, 0, 1);
                  const lineEase = easeOutCubic(local);
                  line.style.opacity = String(lineEase);
                  line.style.transform = `translate3d(0, ${THREE.MathUtils.lerp(18, 0, lineEase).toFixed(1)}px, 0)`;
                  line.style.filter = `blur(${THREE.MathUtils.lerp(4, 0, lineEase).toFixed(2)}px)`;
                }
              }

              // Liquid filling mapped to Phase 2 -> 3 (0.33 to 0.66)
              const liquidT = THREE.MathUtils.clamp((tRaw - 0.33) / 0.33, 0, 1);
